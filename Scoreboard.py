...
class Scoreboard:
    ...
    def load_session(cls, guild_id, user_id):
        ...
        try:
            logging.debug(
                f"Attempting to download session data for guild {guild_id} and user {user_id}"
            )
            data = client.download_as_text(key)
            cls.sessions[(guild_id, user_id)] = json.loads(data)
            logging.debug(
                f"Loaded session data for guild {guild_id} and user {user_id}: {cls.sessions[(guild_id, user_id)]}"
            )
        except requests.exceptions.HTTPError as e:
            if e.response.status_code == 404:
                logging.debug(
                    f"Session data for guild {guild_id} and user {user_id} not found, initializing new session"
                )
                cls.sessions[(guild_id, user_id)] = SESSION_TEMPLATE.copy()
            else:
                logging.error(
                    f"Error loading session data for guild {guild_id} and user {user_id}: {e}"
                )
                raise
    ...
    def save_session(cls, guild_id, user_id):
        ...
        logging.debug(
            f"Saving session data for guild {guild_id} and user {user_id}: {data}"
        )
        client.upload_from_text(key, data)

    @classmethod
    async def get_squadron_points(cls, guild_id):
        try:
            ...
            embed = await fetch_squadron_info(long_hand_name, "points")
            if embed is not None:
                ...
                logging.error(
                    f"Could not fetch points for squadron {long_hand_name}")
                return 0  # Default to 0 if points cannot be fetched

        except Exception as e:
            logging.error(
                f"Error fetching squadron points for guild {guild_id}: {e}")
            return 0  # Default to 0 if an error occurs

    @classmethod
    async def start_session(cls, interaction: discord.Interaction,
                            region: str):
        ...
        if session["started"]:
            await interaction.response.send_message(
                "You already have an active session.", ephemeral=True)
            return
        ...
        squadron_points = await cls.get_squadron_points(guild_id)
        ...
        try:
            ...
            message = await interaction.channel.send(
                f"{session_start_message}\n```diff\n{session['log']}\n```")
            session["last_message_id"] = message.id
            cls.save_session(guild_id, user_id)
            logging.debug(
                f"Session started message sent: {session_start_message}")
        except Exception as e:
    ...
    async def log_game(cls,
        ...
        new_log_entry = cls.format_log_entry(status, session["wins"],
                                             session["losses"],
                                             new_game_number, team_name,
                                             bombers, fighters, helis, tanks,
                                             spaa, f"(updating...) {comment}")
        ...
        try:
            ...
            await interaction.response.send_message(
                "Game logged", ephemeral=True)

        except discord.errors.InteractionResponded:
            logging.error("Interaction has already been responded to")
            await interaction.followup.send("Game logged",
                                            ephemeral=True)

        # Start background task to update points
        asyncio.create_task(
            cls.update_points_in_log(guild_id, user_id, interaction.channel,
                                     message.id))

    @classmethod
    async def update_points_in_log(cls, guild_id, user_id, channel,
                                   message_id):
        ...
        while datetime.datetime.utcnow() - start_time < max_wait_time:
            session["current_points"] = await cls.get_squadron_points(guild_id)
        ...
        try:
            if session["log_entries"]:
                ...
                updated_entry = last_entry.replace("(updating...)",
                                                   point_diff_text)
                ...
                await message.edit(
                    content=
                    f"**{session['region']} SESSION START - <@{session['user']}>**\n```diff\n{session['log']}\n```"
                )
                cls.save_session(guild_id, user_id)
    ...
    async def log_win(cls,
        ...
        await cls.log_game(interaction, "+", team_name, bombers, fighters,
                           helis, tanks, spaa, comment)

    @classmethod
    async def log_loss(cls,
                       interaction: discord.Interaction,
                       team_name: str,
                       bombers: int,
                       fighters: int,
    ...
    @classmethod
    async def end_session(cls, interaction: discord.Interaction,
                          bot: discord.Client):
        guild_id = interaction.guild.id
        user_id = interaction.user.id
        ...
        try:
            ...
            try:
                ...
                if e.response.status_code == 404:
                    logging.warning(
                        f"No active session file found for {session_key}")
                    await interaction.response.send_message(
                        "No active session found to end.", ephemeral=True)
                    return
                else:
                    raise
            ...
            win_rate = (session["wins"] /
                        total_games) * 100 if total_games > 0 else 0

            # Fetch final points
            final_points = await cls.get_squadron_points(guild_id)
            ...
            if session["last_message_id"]:
                try:
                    logging.debug(
                        f"Attempting to delete last session message for guild {guild_id} and user {user_id}"
                    )
                    last_message = await interaction.channel.fetch_message(
                        session["last_message_id"])
                    await last_message.delete()
                    ...
                    logging.warning(
                        f"Last message for guild {guild_id} and user {user_id} not found, it might have already been deleted."
                    )
                except Exception as e:
                    logging.error(
                        f"Error deleting last message for guild {guild_id} and user {user_id}: {e}"
                    )
            ...
            try:
                ...
                logging.error(
                    f"Error sending session end message in guild {guild_id}: {e}"
                )
                if not interaction.response.is_done():
                    await interaction.followup.send(
                        f"An error occurred while ending the session: {e}",
                        ephemeral=True)
                return
            ...
            try:
                ...
                logging.error(
                    f"Error deleting session file {session_key}: {e}")

            if not interaction.response.is_done():
                await interaction.response.send_message(
                    "Session ended and file deleted.", ephemeral=True)

        except Exception as e:
            logging.error(
                f"Unhandled exception while ending session for {session_key}: {e}"
            )
            if not interaction.response.is_done():
                await interaction.followup.send(
                    f"An error occurred while ending the session: {e}",
    ...
    async def edit_game(cls,
        ...
        if session is None or not session[
                "started"] or interaction.user.id != session["user"]:
            await interaction.response.send_message(
                "You are not authorized to edit a game or no session is active.",
                ephemeral=True)
            return
        ...
        if not session["log_entries"]:
            await interaction.response.send_message(
                "No games have been logged yet.", ephemeral=True)
            return
        ...
        try:
            ...
            new_game_number = session["wins"] + session["losses"]
            new_log_entry = cls.format_log_entry(
                new_status, session["wins"], session["losses"],
                new_game_number, team_name, bombers, fighters, helis, tanks,
                spaa, f"{point_diff_text} {comment}")
            ...
            if not interaction.response.is_done():
                await interaction.followup.send(
                    f"An error occurred while editing the game: {e}",
                    ephemeral=True)

    @classmethod
    def format_log_entry(cls,
                         status,
                         wins,
                         losses,
                         game_number,
                         ...
                         spaa,
                         comment=''):
        spacing = "   " if len(str(wins)) == 1 and len(
            str(losses)) == 1 else "  " if len(str(wins)) > 1 and len(
                str(losses)) == 1 else "  " if len(str(wins)) == 1 and len(
                    str(losses)) > 1 else " "
