                        ...
                        if channel_id.isdigit():
                            ...
                            if channel:
                                for member, points in left_members.items():
                                    await channel.send(f"Member {member} left with {points} points")
                            else:
...
async def on_ready():
    ...
    await bot.change_presence(activity=discord.Activity(type=discord.ActivityType.watching, name="Preed"))
    if not bot.synced:
        await bot.tree.sync()
        bot.synced = True
    snapshot_task.start()
...
BR_OPTIONS = [
    "4.3", "4.7", "5.0", "5.3", "5.7", "6.0", "6.3", "6.7", 
    "7.0", "7.3", "7.7", "8.0", "8.3", "8.7", "9.0", "9.3", 
    "9.7", "10.0", "10.3", "10.7", "11.0", "11.3", "11.7",
    "12.0", "12.3", "12.7", "13.0", "13.3", "13.7"
]
...
def view_metalist(br: str, server_id: int) -> discord.Embed:
    ...
    try:
        ...
        if e.response.status_code == 404:
            return discord.Embed(title="Meta List", description="Meta is empty.", color=discord.Color.red())
        else:
            logging.error(f"Error loading meta data for server {server_id}: {e}")
            return discord.Embed(title="Meta List", description="An error occurred while trying to view the Meta list.", color=discord.Color.red())
    except Exception as e:
        logging.error(f"Error loading meta data for server {server_id}: {e}")
        return discord.Embed(title="Meta List", description="There is no Meta list for this server.", color=discord.Color.red())

    metas = json.loads(data)

    if not metas:
        return discord.Embed(title="Meta List", description=f"No vehicles found at BR {br} in Meta.", color=discord.Color.red())
    ...
    embed = discord.Embed(title=f"Meta List for BR {br}", color=discord.Color.blue())
    for category, vehicles in categorized_vehicles.items():
        if vehicles:
            embed.add_field(name=category, value="\n".join(vehicles), inline=False)
...
class InitialDropdown(discord.ui.Select):
    ...
    async def callback(self, interaction: discord.Interaction):
        if self.values[0] in ["Add", "Remove"]:
            if not has_permission(interaction.user.id, interaction.user.roles, "Meta", interaction.guild.id) and not interaction.user.guild_permissions.administrator:
        ...
        if self.values[0] == "Add":
            ...
            await interaction.response.send_message("Select BRs to view:", view=BRSelectView(interaction))
        else:
...
class AddModal(discord.ui.Modal, title="Add Item"):
    ...
    async def on_submit(self, interaction: discord.Interaction):
        search_term = self.children[0].value.lower()  # Convert search term to lowercase
        ...
        for file_path in [GROUND_DATA_FILE, AIR_DATA_FILE]:
            with open(file_path, 'r') as f:
                lines = f.readlines()
                matches.update(normalize_name(line.strip()) for line in lines if search_term in line.lower())  # Convert each line to lowercase for comparison

        matches = list(matches)  # Convert back to list to allow indexing

        if not matches:
            await interaction.response.send_message(f"No matches found for '{search_term}'.", ephemeral=True)
        elif len(matches) == 1:
            view = BRSelectionView(matches[0])
            await interaction.response.send_message(f"Select BRs for '{matches[0]}':", view=view)
        else:
            view = MatchSelectionView(matches)
            await interaction.response.send_message("Multiple matches found, please select one:", view=view)
...
class MatchSelectionView(discord.ui.View):
    ...
    def add_match_dropdowns(self):
        options_chunks = [self.matches[i:i + 25] for i in range(0, len(self.matches), 25)]
...
class MatchSelectionDropdown(discord.ui.Select):
    ...
    async def callback(self, interaction: discord.Interaction):
        ...
        await interaction.response.send_message(f"Select BRs for '{selected_match}':", view=view)
...
class BRSelectionView(discord.ui.View):
    ...
    def add_br_dropdowns(self):
        options_chunks = [BR_OPTIONS[i:i + 25] for i in range(0, len(BR_OPTIONS), 25)]
        for options_chunk in options_chunks:
            self.add_item(BRSelectionDropdown(self.selected_match, options_chunk))
...
class BRSelectionDropdown(discord.ui.Select):
    ...
    async def callback(self, interaction: discord.Interaction):
        ...
        for br in selected_brs:
            response = add_to_metas(self.selected_match, br, interaction.guild.id)
...
class RemoveModal(discord.ui.Modal, title="Remove Item"):
    ...
    async def on_submit(self, interaction: discord.Interaction):
        ...
        if not matches:
            await interaction.response.send_message(f"No matches found for '{search_term}' in meta data.", ephemeral=True)
        else:
            view = RemoveMatchSelectionView(matches, search_term)
            await interaction.response.send_message(f"Select the vehicle to remove:", view=view)
...
class RemoveMatchSelectionView(discord.ui.View):
    ...
    def add_match_dropdowns(self):
        options_chunks = [list(self.matches.keys())[i:i + 25] for i in range(0, len(self.matches), 25)]
        for options_chunk in options_chunks:
            self.add_item(RemoveMatchSelectionDropdown(self.search_term, self.matches, options_chunk))
...
class RemoveMatchSelectionDropdown(discord.ui.Select):
    ...
    async def callback(self, interaction: discord.Interaction):
        ...
        view = RemoveBRSelectionView(self.matches[selected_vehicle], selected_vehicle)
        await interaction.response.send_message(f"Select BRs to remove '{selected_vehicle}' from:", view=view)
...
class RemoveBRSelectionView(discord.ui.View):
    ...
    def add_br_dropdowns(self):
        options_chunks = [self.brs[i:i + 25] for i in range(0, len(self.brs), 25)]
        for options_chunk in options_chunks:
            self.add_item(RemoveBRSelectionDropdown(self.selected_vehicle, options_chunk))
...
class RemoveBRSelectionDropdown(discord.ui.Select):
    ...
    async def callback(self, interaction: discord.Interaction):
        selected_brs = self.values
        # Log the details
        logging.debug(f"Item: {self.selected_vehicle} removed from BRs: {selected_brs}")

        response_messages = []

        # Remove the vehicle from each selected BR using meta_remove
        for br in selected_brs:
            response = remove_from_metas(self.selected_vehicle, br, interaction.guild.id)
    ...
    def add_br_dropdowns(self):
        ...
        try:
            ...
            if e.response.status_code == 404:
                ...
                logging.error(f"Error loading meta data for server {self.interaction.guild.id}: {e}")
                metas = {}
        except Exception as e:
            logging.error(f"Error loading meta data for server {self.interaction.guild.id}: {e}")
            metas = {}
...
class BRSelectDropdown(discord.ui.Select):
    ...
    async def callback(self, interaction: discord.Interaction):
        ...
        embeds = [view_metalist(br, interaction.guild.id) for br in selected_brs]
...
async def session(interaction: discord.Interaction):
    ...
    if current_time >= datetime.time(14, 0) and current_time <= datetime.time(22, 0):
        region = "EU"
    elif current_time >= datetime.time(1, 0) and current_time <= datetime.time(7, 0):
        region = "US"
    else:
        region = "TEST"
    try:
        ...
        if not url:
            ...
            if not url:
                await interaction.followup.send(
                    "No image found in recent messages and no URL provided.", ephemeral=True)
                return
...
@bot.tree.command(
    name="bubble",
    description="Create a GIF with an empty speech bubble from the last image posted or a provided URL")
@app_commands.describe(
    url="The URL of the image to add an empty speech bubble to and turn into a GIF (optional)")
async def bubble(interaction: discord.Interaction, url: str = None):
    ...
    try:
        ...
        await interaction.followup.send(
            f"An error occurred while creating the GIF with speech bubble: {e}", ephemeral=True)
...
async def sq_info(interaction: discord.Interaction, squadron: str, type: str = None):
    await interaction.response.defer(ephemeral=False)
    try:
        ...
        await interaction.followup.send(f"An error occurred while fetching the squadron info: {e}")
...
def update_leaderboard(guild_id, user_id, points):
    ...
    try:
        ...
        logging.warning(f"Leaderboard file for guild {guild_id} not found. Creating a new one.")
...
async def guessing_game_command(interaction: discord.Interaction):
    ...
    logging.debug(f"Selected vehicle: {selected_vehicle}, Normalized Name: {normalized_name}, Image URL: {image_url}")
    ...
    def check(m):
        return m.channel == interaction.channel and m.content.lower() == normalized_name.lower()

    timeout_seconds = 60  # Set timeout duration

    try:
        guess = await bot.wait_for('message', timeout=timeout_seconds, check=check)
        if guess:
            embed = discord.Embed(
                title="Congratulations!",
                description=f"Correct {guess.author.mention}! The vehicle is **{normalized_name}**",
                color=discord.Color.green()
            )
            await interaction.followup.send(embed=embed)
        ...
        embed = discord.Embed(
            title="Guessing Game",
            description=f"Time's up! You took too long to respond. The vehicle was **{normalized_name}**",
            color=discord.Color.red()
        )
        await interaction.followup.send(embed=embed)
...
async def leaderboard_command(interaction: discord.Interaction):
    ...
    try:
        ...
        embed.description = "\n".join([f"<@{user_id}>**: {score} points**" for user_id, score in sorted_leaderboard])
...
def modify_vehicles(vehicles, count):
    ...
    for vehicle in fake_vehicles:
        ...
        if any(char.isdigit() for char in vehicle_name):
            modified_name = ''.join([str((int(char)+1) % 10) if char.isdigit() else char for char in vehicle_name])
        else:
        ...
        logging.debug(f"Modified vehicle name: {modified_name}, Normalized modified name: {normalized_modified_name}")
...
class TriviaDropdown(ui.Select):
    def __init__(self, vehicles, fake_vehicles, fake_count, difficulty, interaction):
        options = [SelectOption(label=normalize_name(vehicle[0]), value=normalize_name(vehicle[0])) for vehicle in vehicles]
        random.shuffle(options)  # Shuffle the options to randomize their order
        super().__init__(placeholder="Select the fake vehicles...", max_values=fake_count, options=options)
    ...
    async def callback(self, interaction: Interaction):
        if interaction.user != self.interaction_user:
            await interaction.response.send_message("Only the user who started the game can play.", ephemeral=True)
            return

        chosen_fakes = self.values
        true_fakes = [normalize_name(vehicle[0]) for vehicle in self.fake_vehicles]
        ...
        correct_count = sum([1 for fake in chosen_fakes_sorted if fake in true_fakes_sorted])

        if correct_count == self.fake_count and correct_count == len(chosen_fakes):
            points = {"easy": 1, "medium": 2, "hard": 3}[self.difficulty]
            update_leaderboard(interaction.guild.id, interaction.user.id, points)
            embed = discord.Embed(
                title="Congratulations!",
                description=f"Correct {interaction.user.mention}! You found all the fake vehicles.",
                color=discord.Color.green()
            )
            await interaction.response.send_message(embed=embed)
        else:
            ...
            embed = discord.Embed(
                title="Try Again!",
                description=f"You have {remaining_fakes} fake vehicles left. Try again!",
                color=discord.Color.red()
            )
            await interaction.response.send_message(embed=embed)
...
class TriviaView(ui.View):
    def __init__(self, vehicles, fake_vehicles, fake_count, difficulty, interaction):
        super().__init__()
        self.add_item(TriviaDropdown(vehicles, fake_vehicles, fake_count, difficulty, interaction))
        self.fake_vehicles = fake_vehicles
...
async def trivia(interaction: Interaction, difficulty: str = "medium"):
    ...
    if difficulty not in difficulty_levels:
        await interaction.response.send_message("Invalid difficulty level. Please choose from: easy, medium, or hard.", ephemeral=True)
        return
    ...
    if len(vehicles) < 10:
        await interaction.response.send_message("Not enough vehicles in the files to start the trivia.")
        return

    modified_vehicles, fake_vehicles = modify_vehicles(vehicles, fake_count)

    view = TriviaView(modified_vehicles, fake_vehicles, fake_count, difficulty, interaction)
    view.message = await interaction.response.send_message("Select the fake vehicles from the dropdown below:", view=view)
...
async def help(interaction: discord.Interaction):
    ...
    embed = discord.Embed(title="Bot Guide", description=guide_text, color=discord.Color.blue())
    await interaction.response.send_message(embed=embed)
...
async def handle_attachment(interaction: discord.Interaction, attachment: discord.Attachment, enemy_team: str, message: discord.Message, result_type: str, comment: str):
    if attachment.filename.endswith('.txt'):
        ...
        try:
            ...
            if games:
                ...
                categorized_vehicles = {'Ground Forces': 0, 'Anti-Aircraft': 0, 'Helis': 0, 'Bombers': 0, 'Air Forces': 0, 'Unknown': 0}
...
@bot.tree.command(name='quick-log', description='Process game logs with status and enemy team')
@app_commands.describe(status='Win or Loss (W/L)', enemy_team='Name of the enemy team', comment='Additional comments')
@has_roles_or_admin("Session")
async def quick_log(interaction: discord.Interaction, status: str, enemy_team: str, comment: str = ""):
    # Ensure status is either 'W' or 'L'
    if status not in ['W', 'L']:
        await interaction.response.send_message("Invalid status value. Please use 'W' or 'L'.", ephemeral=True)
        return

    result_type = 'win' if status == 'W' else 'loss'
    await interaction.response.send_message("Click this [link](http://localhost:8111/hudmsg?lastEvt=0&lastDmg=0), copy all contents, and paste them back below this message.", ephemeral=True)
    ...
    if message.attachments:
        await handle_attachment(interaction, message.attachments[0], enemy_team, message, result_type, comment)
...
async def chatgpt(interaction: discord.Interaction, message: str):
    ...
    try:
        ...
        await interaction.followup.send(
            f"An error occurred while getting the response from ChatGPT: {e}", ephemeral=True)
...
async def on_message(message):
    ...
    if message.reference and message.reference.message_id in message_references:
        ...
        try:
            ...
            await message.channel.send(f"An error occurred while getting the response from ChatGPT: {e}")
